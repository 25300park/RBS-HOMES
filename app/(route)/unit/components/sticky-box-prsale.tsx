...
export interface StickyBoxPresaleProps {
  sellType: string;
...
const StickyBoxPresale = ({
  price,
  sellType,
  unitId,
}: StickyBoxPresaleProps): React.ReactNode => {
  const { data: session } = useSession();
  const [date, setDate] = useState<Date>();
  const [needsDiscussion, setNeedsDiscussion] = useState(true);
  ...
  return (
    <div className="w-[350px] border sticky top-24 self-start p-6 shadow-lg rounded-lg flex flex-col gap-4 md:hidden">
...
export default StickyBoxPresale;
